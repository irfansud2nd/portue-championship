...
import {
  getKontingensScoreByCreator,
  writeAllKontingen,
} from "@/utils/scoring/scoringActions";
...
export const ScoringContextProvider = ({
  ...
  const refreshKontingens = async () => {
    try {
      ...
      const { result, error } = await getKontingensScoreByCreator(user.email);
  ...
  const initializeKontingens = async () => {
    try {
      setInitializeLoading(true);
      const { result, error } = await writeAllKontingen(user);
      if (error) throw error;
    } catch (error) {
      alert(error);
    } finally {
